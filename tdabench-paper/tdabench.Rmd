...
abstract: >
  With the increasing popularity of topological data analysis, several persistent homology software libraries have been implemented in R. The Dionysus and GUDHI C++ libraries have been implemented in the TDA package, and the Ripser C++ library has been implemented in the TDAstats package. All libraries feature Rips complex functionality. Additionally, within the TDA GUDHI package, the Alpha complex is available. These software represent powerful tools for analyzing the underlying topological structure of data. However, they are limited by computational restraints. High dimensional features and large point cloud sizes are limiting factors for persistent homology calculations. These limitations and guidelines for which persistent homology libraries to use have not been specified in R. To fill this gap, we benchmarked these libraries on a High-Powered Cluster. We benchmarked all Rips complex libraries for runtime. In addition, we compared Alpha and Rips for memory burden and runtime. We find that Alpha complexes run significantly faster than all Rips complex libraries for all data sets where data dimension is less than or equal to three. Alpha complexes were not computable on our systems for greater than 3 dimensions. For Rips complex calculations, run time performance was fastest for TDAstats and slowest for Dionysus. This difference was markedly clear at high dimensional features and point cloud values. From a computational efficiency perspective, if data dimension is less than or equal to 3, we recommend performing persistent homology with GUDHI Alpha complexes. If data dimension is higher, we recommend using TDAstats Rips Complex. 
output:
...
In our analysis, we compare runtime performances of the three Rips complexes libraries for persistence homology calculation under different conditions. We then focus our analysis on comparing Rips complexes with Alpha complexes using the GUDHI package.
...
A persistent homology calculation of a uniformly distributed torus would reveal a large 1 dimensional feature representing the middle circle of the torus. There would also be a large 2 dimensional feature representing the inner cavity of the torus. 
In figure \ref{fig:tor}, we calculated up to 2 dimensional features of torus 3d points cloud for an increasing number of points across all TDA libraries. 
