...
\begin{figure}
  ...
  \caption{
    ...
    Average runtime (mean $\pm$ SD, $n=10$) for each TDA library (denoted by color) is plotted against point cloud size.
...
Tori do not trivially generalize to dimensions other than 3, but circles do.
...
Figure \ref{fig:cir} exhibits the resulting data for a 2-dimensional circle (panel A), 3-dimensional sphere (panel B), and 4-dimensional hypersphere (panel C).
...
\begin{widefigure}
  ...
  \caption{
    ...
    Runtime of persistent homology calculation as a function of underlying engine for different dimension circles.
    The x-axis represents the number of points in the point cloud, and y-axis represents average run time.
    Each library that calculated the Rips complex is differentiated by point color.
    All calculations calculated up to two dimensional features.
    Panel 1 compares library performance for a 2-dimensional circular point cloud; panel 2 for a 3-dimensional spherical point cloud; and panel 3 for a 4-dimensional hyperspherical point cloud.
    Maximum feature dimensions (one less than data dimension) were calculated for each point cloud.
    Average runtime is presented as mean $\pm$ SD over 10 iterations.
  }
\label{fig:cir}
\end{widefigure}
