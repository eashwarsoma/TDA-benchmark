...
The exact values for the log-log regression of this growth pattern is provided in supplemental data. 
...
We provide the linear regression coefficients in the supplemental data. 
...
\begin{figure}
  ...
  \caption{Persistent Homology Boundary Matrix for Rips vs Alpha complex. Shape of used point cloud is demonstrated by each image. Rips complex vs Alpha complex for object size.}
\label{fig:mem1}
\end{figure}
...
\begin{figure}
  ...
  \caption{Different factors change boundary matrix size for each complex. Panel 1 = Rips complex object size depends on feature size and point cloud number alone. Panel 2 = Alpha complex object size depends on feature size, point cloud number, and point cloud shape. Point cloud shape is not defined as it had no impact on boundary matrix size. For a given n point cloud, n-1 dimension features were calculated in these examples.}
\label{fig:mem2}
\end{figure}
