...
Essentially, we measures features that are "discovered" by the algorithm at a particular stage and disappear at a later stage. 
...
An $n$-dimensional simplex is the convex hull of $n+1$ points in some Euclidean space.
...
In a simplicial complex, topological features will arise that can be characterized by Betti numbers. Each Betti number $k$ counts the number of features in dimension $k$. Betti $0$ counts the number of connected components, Betti $1$ counts refer to loops, Betti $2$ counts the number of voids, and so on \citep{phom-survey}.
...
For point clouds in 3 dimensions, spheres of diameter $\delta$ are drawn around each point, and hyperspheres of appropriate dimensions are drawn in higher dimensional spaces. 
The remainder of this explanation will consider the 2 dimensional case of circles. 
...
When a triple of points is connected, we add a triangle (2-simplex). When a quadruple of points is connected, we add tetrahedron (3-simplex) and so forth. 
...
<!--Is below theoretical max accurate phrasing?-->
...
Fo alpha complexes, we partition the whole space in which the data resides into cells such that each cell contains exactly one data point x, and the cell of that data point is the set of all points closer to x than any other data points.
...
This is because alpha complexes satisfy the Nerve theorem ~\citep{alpha-complex}. 
This essentially means that alpha complexes are topologically equivalent to the true underlying topology of the datase whereas Rips complexes are approximations ~\citep{Rips-Complex}.
