...
Persistent features more likely represent real geometric patterns, rather than noise.
...
In a simplicial complex, certain features will arise that can be characterized by Betti number: 0-dimensional features refer to connected components, 1-dimensional features refer to loops, 2-dimensional features refer to voids, and so on \citep{phom-survey}.
...
In a point cloud of $k$ points, a distance parameter, $\delta$, can be used to draw a circle of diameter $\delta$ around every point in $S$.
...
A 3-column matrix can be created recording the Betti number of each feature, the $\delta$ at which that feature appeared, and the $\delta$ at which it disappeared.
...
Alpha complex calculations have a complexity of $O(n^{d/2})$ and Vietoris-Rips complex calculations have a complexity of $O(2^n)$ where $n$ is the number of points and $d$ is the point cloud dimension \citep{roadmap}.
